...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (count($attempts[$ip]) >= $LOGIN_ATTEMPT_LIMIT) {
        $popup = ['type' => 'error', 'message' => 'Security Lockout: Too many attempts. Wait 15m.'];
    } else {
        $attempts[$ip][] = $now;
        ...
        if ($identifier === '' || $password === '') {
            $popup = ['type' => 'error', 'message' => 'Please enter credentials.'];
        } else {
            // Database Check
            $stmt = $conn->prepare('SELECT id, password FROM contacts WHERE email = ? OR phone = ? LIMIT 1');
            if ($stmt) {
                ...
                if ($row = $result->fetch_assoc()) {
                    if (password_verify($password, $row['password'])) {
                        ...
                        $popup = ['type' => 'success', 'message' => 'Client Authorized. Accessing Portal...'];
                    } else {
                        $popup = ['type' => 'error', 'message' => 'Authentication Failed.'];
                    }
                } else {
                    $popup = ['type' => 'error', 'message' => 'Client record not found.'];
                }
                $stmt->close();
            } else {
                $popup = ['type' => 'error', 'message' => 'System error: Database unavailable.'];
            }
        }
    }
} else {
...
<head>
    ...
    <title>Client Portal | JP Construction</title>
    ...
    <style>
        :root {
            ...
            --accent: #10b981;        /* Emerald Green for Clients */
            --text-main: #f8fafc;
            --text-muted: #94a3b8;
        }
...
<body>
    ...
    <div class="login-wrapper">
        <div class="glass-card">
            ...
            <h2 class="brand-title">Client Access</h2>
            <p class="brand-subtitle">Project & Contact Management</p>
            ...
            <form method="POST" autocomplete="off">
                ...
                <button type="submit" class="btn-submit">
                    Secure Login <i class="fa-solid fa-shield-halved ms-2"></i>
                </button>
            ...
            <a href="../index.php" class="back-link">
                <i class="fa-solid fa-house me-1"></i> Return to Main Site
            </a>
