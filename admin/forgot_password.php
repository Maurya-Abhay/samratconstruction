    ...
    try {
        ...
        if ($_POST['ajax_action'] === 'send_otp') {
            ...
            if ($res && $res->num_rows) {
                ...
                if ($stmt2->execute()) {
                    ...
                    // --- SEND EMAIL (Brevo) ---
                    $apiKeyPath = __DIR__.'/brevo_api_key.txt';
                    // Use setting or fallback to file
                    $sender = get_setting('brevo_sender', trim(@file_get_contents(__DIR__.'/brevo_sender.txt') ?: 'no-reply@example.com'));
                    
                    if (file_exists($apiKeyPath)) {
                        $apiKey = trim(file_get_contents($apiKeyPath));
                        
                        $subject = '🔐 Admin Reset Code';
                        $body = "<div style='font-family:sans-serif;padding:20px;border:1px solid #eee;border-radius:10px;background:#f9f9f9;'>"
                            . "<h2 style='color:#0d6efd;text-align:center;'>Admin Recovery</h2>"
                        ...
                        $data = [
                            'sender' => ['email' => $sender, 'name' => 'Admin Security'],
                            'to' => [['email' => $email]],
                            'subject' => $subject,
                            'htmlContent' => $body
                        ];
        ...
        elseif ($_POST['ajax_action'] === 'reset_password') {
            ...
            if ($res && $row = $res->fetch_assoc()) {
                ...
                $u = $conn->prepare("UPDATE admin SET password = ? WHERE email = ? LIMIT 1");
...
<head>
    ...
    <title>Admin Recovery | Samrat Construction</title>
    ...
    <style>
        ...
        .form-floating > label { padding-top: 10px; }
...
<body>
    ...
    <div class="auth-card">
        <div class="auth-header">
            ...
            <h4 class="fw-bold mb-1">Admin Recovery</h4>
            <p class="text-muted small mb-0">Secure Admin Access</p>
        </div>
        ...
        <div class="auth-body">
            ...
            <div id="view-1" class="step-view active">
                <form id="formStep1" onsubmit="handleSendOtp(event)">
                    <div class="form-floating mb-4">
                        <input type="email" class="form-control" id="emailInput" placeholder="admin@example.com" required>
                    ...
                    <div class="text-center">
                        <a href="index.php" class="back-btn"><i class="bi bi-arrow-left me-1"></i> Back to Login</a>
                    </div>
                </form>
            </div>
            ...
            <div id="view-3" class="step-view text-center pt-3">
                ...
                <p class="text-muted small mb-4">Your admin password has been reset.</p>
            ...
            try {
                ...
                if (res.status === 'success') {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'OTP Sent',
                        text: 'Check your admin email inbox.',
                        toast: true,
                        position: 'top',
